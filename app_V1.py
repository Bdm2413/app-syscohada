...
from balance import generer_balance
import base64
...
# Grand Livre
    ...
    if not st.session_state.data_loaded:
        ...
        mois_filter = st.sidebar.multiselect("Mois (AAAAMM)", options=sorted(gl_df["Mois"].dropna().unique()))
...
def bilan():
    ...
    annee_n = st.sidebar.selectbox("Année N", options=["2024", "2023", "2022"], index=0)
    annee_n1 = st.sidebar.selectbox("Année N-1", options=["2023", "2022", "2021"], index=1)

    st.title("Bilan")

    # Exemple de données fictives pour illustrer le remplissage des colonnes Année N et N-1
    def build_bilan_df(structure):
        df = pd.DataFrame(structure)
        df["Année N"] = ""  # à remplacer par les vraies données plus tard
        df["Année N-1"] = ""
        return df

    df_actif = build_bilan_df(structure_bilan_actif)
    df_passif = build_bilan_df(structure_bilan_passif)

    st.subheader("Bilan Actif")
    st.dataframe(df_actif, hide_index=True)

    st.subheader("Bilan Passif")
    st.dataframe(df_passif, hide_index=True)

# Appel de la fonction selon le menu sélectionné
